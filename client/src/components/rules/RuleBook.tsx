...
import { BookOpen, ChevronDown, Search, ShieldCheck, Sparkles, AlertCircle } from 'lucide-react';
    ...
    <section id="rules" className="py-20 px-4 max-w-5xl mx-auto relative">
      ...
      <div className="text-center mb-12">
        <div className="inline-flex items-center gap-2 text-carnival-crimson font-mono text-xs uppercase tracking-widest mb-2 font-semibold px-4 py-1.5 rounded-full bg-carnival-crimson/10 border border-carnival-crimson/30">
          ...
          <span>Official Season 4 Codex</span>
        </div>
        <h2 className="text-3xl sm:text-5xl font-black tracking-tight mb-4">
          Carnival <span className="text-gradient-carnival">Rule Book</span>
        </h2>
        <p className="text-slate-300 text-sm sm:text-base max-w-xl mx-auto">
          Read all ground regulations, scoring rules, penalty guidelines, and power-up card usage rules before stepping into the arena.
        </p>
      ...
      {/* Accordion List of Rule Categories */}
      <div className="space-y-4 relative z-10">
        {MOCK_RULES.map((category: RuleCategory) => {
          ...
          return (
            <div
              key={category.id}
              className="glass-card rounded-2xl border border-white/10 overflow-hidden shadow-xl transition-all"
            >
      ...
      <div className="mt-8 p-4 rounded-xl glass-card border border-carnival-crimson/30 flex items-start gap-3 text-xs text-slate-300">
        ...
        <div>
          <strong className="text-white font-semibold">Note on Disqualification:</strong> Violation of code submission integrity or attempting unauthorized server API manipulations will result in instant team forfeiture and permanent ban from CWC tournaments.
        </div>
      </div>
    </section>
  );
