...
export const AdminLayout: React.FC = () => {
  ...
  return (
    <div className={`min-h-screen bg-slate-50 dark:bg-[#0B0A16] text-slate-900 dark:text-slate-100 flex font-sans transition-colors duration-300 selection:bg-carnival-gold selection:text-black ${isGrandFinale ? 'grand-finale-gold' : ''}`}>
      {/* Admin Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 z-40 w-64 h-screen bg-white/95 dark:bg-[#1A1228]/95 backdrop-blur-2xl border-r border-slate-200 dark:border-carnival-crimson/30 flex flex-col justify-between p-4 transition-transform duration-300 overflow-y-auto ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div className="flex-1 flex flex-col min-h-0">
          {/* Brand & Admin Badge */}
          <div className="flex items-center gap-3 pb-6 border-b border-slate-200 dark:border-white/10 mb-6 shrink-0">
          ...
          <div className={`p-3 rounded-xl border mb-6 font-mono text-xs transition-all shrink-0 ${
          ...
          <div className="flex-1 overflow-y-auto pr-1 space-y-6">
            ...
            {user?.role === 'superadmin' && (
              <div className="pt-4 border-t border-slate-200 dark:border-purple-500/20">
        ...
        {/* Footer info & Exit */}
        <div className="pt-4 border-t border-slate-200 dark:border-white/10 space-y-2 shrink-0">
          <button
            onClick={logout}
