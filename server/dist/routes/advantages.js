 ...
 */
export async function approveTeamForSpin(request, reply) {
 ...
 */
export async function getSpinWheelStatus(request, reply) {
    const user = request.user;
    const todayDate = getTodayDateString();
    let userTeamId = null;
 ...
 */
export async function handleSpinWheel(request, reply) {
    const user = request.user;
    ...
    if (!winningTeam.advantages)
        winningTeam.advantages = [];
    const existingAdv = winningTeam.advantages.find((a) => a.advantage.toLowerCase() === targetAdvantage.toLowerCase());
    if (existingAdv) {
    ...
    if (!winningTeam.spunDays)
        winningTeam.spunDays = [];
    ...
    await Score.findOneAndUpdate({ team: winningTeam._id, dayNumber }, {
 ...
 */
export async function getActiveTeamsForWheel(_request, reply) {
...
export async function advantagesRoutes(fastify) {
    ...
    fastify.post('/spin-wheel/approve', { preHandler: [isAdmin] }, approveTeamForSpin);
}
