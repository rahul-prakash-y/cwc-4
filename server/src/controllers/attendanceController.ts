...
/* ==========================================================================
   ...
   ========================================================================== */
export async function enforceAttendanceRules(teamId: string, memberIdsPresent: string[], dayNumber: number) {
    ...
    if (team && team.status !== 'Eliminated' && team.status !== 'Danger') {
      ...
      if (team.leader?.email) {
        ...
        setImmediate(() => {
          ...
          sendCarnivalEmail(
            leaderEmail,
            `⚠️ Alert: Danger Status Triggered (Day ${dayNumber} Attendance Rule) - Team ${teamName}`,
            html
          );
        });
      }
...
/* ==========================================================================
   ...
   ========================================================================== */
interface SingleAttendanceItem {
  teamId: string;
...
interface SaveAttendanceBody {
  ...
  memberIdsPresent?: string[];
  isTeamPresent?: boolean;
  attendanceBatch?: SingleAttendanceItem[];
}
...
export async function saveAttendance(
  ...
  const body = (request.body || {}) as SaveAttendanceBody;
      ...
      if (team.status !== 'Eliminated' && team.status !== 'Danger') {
        ...
        if (team.leader?.email) {
          setImmediate(() => {
            ...
            sendCarnivalEmail(
              team.leader.email,
              `⚠️ Alert: Danger Status Triggered (Attendance Rule) - Team ${team.teamName}`,
              html
            );
          });
        }
